...
generate_config() {
    ...
    # Build sed command dynamically based on which endpoints are provided
    local sed_cmd="sed"
    [[ -n "$loki_endpoint" ]] && sed_cmd+=" -e 's|LOKI_ENDPOINT_PLACEHOLDER|${loki_endpoint}|g'"
    [[ -n "$mimir_endpoint" ]] && sed_cmd+=" -e 's|MIMIR_ENDPOINT_PLACEHOLDER|${mimir_endpoint}|g'"
    [[ -n "$tempo_endpoint" ]] && sed_cmd+=" -e 's|TEMPO_ENDPOINT_PLACEHOLDER|${tempo_endpoint}|g'"
    sed_cmd+=" '$template'"
...
# Generate alloy-logs stage overlay from template (only LOKI_ENDPOINT)
generate_config \
...
# Generate alloy-logs prod overlay from template (only LOKI_ENDPOINT)
generate_config \
...
# Generate alloy-metrics stage overlay from template (only MIMIR_ENDPOINT)
generate_config \
...
# Generate alloy-metrics prod overlay from template (only MIMIR_ENDPOINT)
generate_config \
...
# Generate alloy-traces stage overlay from template (only TEMPO_ENDPOINT)
generate_config \
...
# Generate alloy-traces prod overlay from template (only TEMPO_ENDPOINT)
generate_config \
...
echo ""
echo -e "${GREEN}Note: Each Alloy agent only sends to its primary backend:${NC}"
echo "  - alloy-logs: sends logs to Loki"
echo "  - alloy-metrics: sends metrics to Mimir"
echo "  - alloy-traces: sends traces to Tempo"
echo ""